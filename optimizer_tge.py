...
class SpectralGlobalOptimizer:
    ...
    def loss_function_v16(self, params: np.ndarray) -> float:
        """
        Função de perda variacional TGE-16.0 ToE (Travamento de Fase Espectral a_12 + Maré de 3 Corpos).
        """
        alpha1, alpha2, beta, gamma_a6, delta_kuiper, xi_resonance, zeta_tidal, eta_phase, theta_locking, escala_base, kappa_sm = params

        if (alpha1 <= 0 or alpha2 <= 0 or beta < 0 or gamma_a6 < 0 or
            delta_kuiper < 0 or xi_resonance < 0 or zeta_tidal < 0 or eta_phase < 0 or theta_locking < 0 or escala_base <= 0 or kappa_sm < 0):
            return 1e6

        res = self.engine.predict_orbits_v16(self.eigenvalues, params.tolist())
        erro_medio = res["erro_medio_global"]

        fator_weyl = self.engine.compute_weyl_correction_v16(
    ...
    def run_optimization_v16(self) -> Dict[str, Any]:
        """
        Executa a busca global variacional multivariada para o modelo TGE-16.0 ToE (Sub-2.5%).
        """
        # Ponto inicial refinado próximo às regiões de ressonância ideal
        p0 = np.array([0.985, 1.095, 0.148, 0.022, 0.865, 0.115, 0.082, 0.179, 0.199, 0.339, 0.000085])
        ...
        bounds = [
            ...
            (0.0, 0.4),    # xi_resonance
            (0.0, 0.3),    # zeta_tidal
            (0.0, 0.4),    # eta_phase (a_12)
            (0.0, 0.3),    # theta_locking (Fase Saturno)
            (0.2, 0.8),    # escala_base
            (0.0, 0.001)   # kappa_sm
        ]

        print("[Otimizador Espectral] Minimizando Ação Espectral TGE-16.0 (ToE Ultra-Precisão Sub-2.5%)...")
        opt_res = minimize(
            self.loss_function_v16,
            p0,
            method="L-BFGS-B",
            bounds=bounds,
        ...
        params_otimos = opt_res.x.tolist()
        res_final = self.engine.predict_orbits_v16(self.eigenvalues, params_otimos)
        ...
        return {
            ...
            "parametros_otimos": {
                ...
                "xi_resonance (Laplace)": params_otimos[5],
                "zeta_tidal (Maré 3 Corpos)": params_otimos[6],
                "eta_phase (Seeley-DeWitt a_12)": params_otimos[7],
                "theta_locking (Fase Saturno)": params_otimos[8],
                "escala_base": params_otimos[9],
                "kappa_sm (Eletrofraco)": params_otimos[10]
            },
            "erro_medio_otimizado": res_final["erro_medio_global"],
            "resultados_orbitais": res_final
        }
...
if __name__ == "__main__":
    ...
    resultado = opt.run_optimization_v16()

    print("=" * 72)
    print("RESULTADO DA OTIMIZAÇÃO VARIACIONAL TGE-16.0 (ToE ULTRA-PRECISÃO)")
    ...
    print(f"{'Planeta':<10} | {'Real (UA)':<12} | {'TGE-16 (UA)':<12} | {'Erro (%)':<10}")
    print("-" * 52)
